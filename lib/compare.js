...
/**
 * Origin used when the bundle was captured. Older dumps omit `origin`.
 *
 * @param {Object|null} bundle
 * @return {string}
 */
function originFromBundle( bundle ) {

	if ( bundle && bundle.origin )
		return String( bundle.origin ).replace( /\/$/, '' );

	for ( const rec of Object.values( ( bundle && bundle.entries ) || {} ) ) {
		const hay = [
			rec.extracted,
			rec.headers && rec.headers.location,
		].filter( Boolean ).join( '\n' );
		const match = hay.match( /https?:\/\/[^/\s"'<>]+/ );
...
function compare( plugin, flags ) {
	...
	for ( const id of ids ) {
		...
		const left  = filterRecord(
			normalize( recordText( a ), originFromBundle( leftB ), plugin.strips ),
			flags.feature,
			plugin,
		);
		const right = filterRecord(
			normalize( recordText( b ), originFromBundle( rightB ), plugin.strips ),
			flags.feature,
			plugin,
		);
